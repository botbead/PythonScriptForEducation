...
key_hit_count = 0
def backgroud_thread_function():
    global key_hit_count,key_hit_count_last,background_color_stack
    while exit is not True:
        time.sleep(1/10.0)
        w = key_hit_count-key_hit_count_last
        key_hit_rate = (key_hit_count - key_hit_count_last) / 5.0
        key_hit_count_last = key_hit_count
        if 0 != w:
            x = 243 - ((243-124) * key_hit_rate)
            y = 145 + ((184-145) * key_hit_rate)
            z = 169 - ((169-14) * key_hit_rate)
            screen.fill((x,y,z))
...
backgroud_thread = threading.Thread(target=backgroud_thread_function)
...
while exit is not True:
    ...
    w = key_hit_count - key_hit_count_last
    key_hit_rate = (key_hit_count - key_hit_count_last) / 3.0
    key_hit_count_last = key_hit_count
    if 0 != w:
        if background_color_stack:
